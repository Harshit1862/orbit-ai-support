...
- **Everywhere a customer might ask.** An animated bot launcher (it wiggles and shows a "Need help?" teaser until first opened) sits on the public homepage for visitors, where it answers pre-sales questions and hands off by email, and on every app screen for signed-in customers.
...
| Help centre | `/help` | The assistant as a full page, with conversation history |
...
flowchart LR
    subgraph Browser
      W[Help widget / help centre<br/>useSupportChat hook]
...
- **Browser.** One hook, `useSupportChat`, holds all the chat logic: sending, retries, timeouts, the rate-limit countdown and saving to storage. `SupportWidget` is the floating launcher and panel built on it; the homepage (`PublicHelpWidget`) and the app (`HelpWidget`) each supply their own greeting, suggestions and handoff. The full-page help centre is a third view on the same hook. `OrbitProvider` holds the workspace the server loaded and sends every change to the server.
...
│   ├── help/                     # Full-page help centre
│   ├── login/                    # Demo login and forgot-password flow
│   └── page.tsx                  # Landing page
├── components/
│   ├── support/                  # Assistant UI: useSupportChat hook, SupportWidget launcher, homepage widget, HelpCenter, shared parts
